 ...
 *
 * $Id: cfg_level.c,v 1.23 2006/03/28 11:41:19 fzago Exp $
 *
 * Program XBLAST
...
{
	return strcmp (((const LevelTable *) a)->key.name, ((const LevelTable *) b)->key.name);
}								/* CompareSorted */
...
{
	return ((const LevelTable *) a)->key.value - ((const LevelTable *) b)->key.value;
}								/* CompareShuffle */
...
{
	return ((const LevelTable *) a)->key.lastplayed - ((const LevelTable *) b)->key.lastplayed;
}								/* CompareTime */
...
GetLastIndexShuffled (void)
{
	...
	Dbg_Level ("update max value to %d\n", max);
...
GetLastIndexTime (void)
{
	...
	for (i = 0; i < numLevels; i++) {
		Dbg_Out ("Lastplayed time of level %s = %s\n", GetLevelNameByAtom (timeTable[i].level),
				 DateString (timeTable[i].key.lastplayed));
	}
#endif
	return numLevels;
}								/* GetLastIndexTime */
	...
	do {
		*pIndex = *pIndex + 1;
		...
		/* check game mode */
			 ...
			 (!allLevels && !GetLevelSelected (atom)));
	/* mark as last level */
...
GetNextLevelFromTable2 (LevelTable * table, int *pIndex, XBAtom entry, void (*pFunc) (void))
{
	...
	for (i = 0; i < numLevels; i++) {
		atom = table[i].level;
		assert (ATOM_INVALID != atom);
		if (gameMode == (gameMode & GetLevelGameMode (atom))
			&& (allLevels || GetLevelSelected (atom))) {
			if (cnt == 0) {
	...
	while ((i < numLevels) && (l > 0)) {
		...
		assert (ATOM_INVALID != atom);
		if (gameMode == (gameMode & GetLevelGameMode (atom))
			&& (allLevels || GetLevelSelected (atom)) && (table[i].key.lastplayed <= t1)) {
			l--;
		}
		i++;
	}
...
IOBToSection (XBTeleIOB iob)
{
	...
	case IOB_SHRINK:
		return atomShrink;
	case IOB_SCRAMBLE_DRAW:
		return atomScrambleDraw;
	case IOB_SCRAMBLE_DEL:
		return atomScrambleDel;
	case IOB_FUNC:
		return atomFunc;
	case IOB_BOMBS:
		return atomBombs;
...
LoadLevelConfig (void)
{
	...
	Dbg_StartClock ();
#endif
	/* create empty level database */
	dbLevel = DB_Create (DT_Config, atomLevel);
	assert (dbLevel != NULL);
	/* scan search paths */
	Dbg_Config ("loading list of levels\n");
	if (DB_LoadDir
		(dbLevel, GAME_DATADIR "/level", "xal", DT_Level, atomTime, atomInfo, InsertLevel,
		 XBTrue)) {
		SaveLevelConfig ();
	}
	else {
    ...
    /* AbsInt start */
    /* load list of unwanted levels and keep them in mind */
    {
      XBAtom atom;
      int c;
      int len;
      char buffer[256];
      FILE *fp=fopen(GAME_DATADIR"/aborted_levels.txt", "r");
      if (!fp)
        return;

      //fprintf(stderr, "number of levels loaded: %d\n", DB_NumSections(dbLevel));
      ...
      while (!feof(fp)) {
          ...
          atom=GUI_StringToAtom(buffer);

          // determine section to manipulate
          for (c=0; c<DB_NumSections(dbLevel); c++) {
             XBAtom a=GetLevelAtom(c);
             const DBSection *section=DB_GetSection(dbLevel, a);
             XBAtom n;
             DB_GetEntryAtom(section, atomName, &n);

             if (n == atom) {
                //fprintf(stderr, "unselecting level '%s' (%d) \n", buffer, atom);
                StoreLevelSelected(a, XBFalse);
                break;
             }
          }
      }
      ...
      //fprintf(stderr, "number of levels after patching: %d\n", DB_NumSections(dbLevel));
    }
    /* AbsInt end */
}								/* LoadLevelConfig */
...
InitLevels (const CFGGame * cfgGame)
{
	...
	case 2:
		...
		break;
	case 3:
		gameMode |= GM_3_Player;
		break;
	case 4:
		gameMode |= GM_4_Player;
		break;
	case 5:
		gameMode |= GM_5_Player;
		break;
	case 6:
		gameMode |= GM_6_Player;
		break;
	}
	/* team mode */
	gameMode |= GM_Single;
...
GetNextLevel (void)
{
	if (levelOrder == 2) {		// LRF
		...
		return GetNextLevelFromTable (shuffledTable, &indexShuffled, atomLevelShuffled,
									  ReshuffleLevels);
	}
...
CheckAllLevelConfigs (void)
{
	...
	do {
		...
		Dbg_Out ("+++LEVEL+++ current = %s(%s)!\n", GUI_AtomToString (atom),
				 GetLevelNameByAtom (atom));
