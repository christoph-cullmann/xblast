 ...
 *
 * $Id: mi_int.c,v 1.5 2006/02/09 21:21:24 fzago Exp $
 *
 * Program XBLAST 
...
MenuIntegerPoll (XBMenuItem * ptr)
{
	...
	assert (string != NULL);
	string->pollCount++;
	if ((string->item.flags & MIF_SELECTED) && (0 == (string->pollCount % BLINK_RATE))) {
	...
	while (1) {
		...
		while (XBE_TIMER != (event = GUI_WaitEvent (&data))) {
			if (event == XBE_ASCII) {
				...
				if (isdigit (data.value) && integer->pos < sizeof (integer->work) - 1) {
				...
				case XBCK_RETURN:
					if (1 == sscanf (integer->work, "%d", integer->pValue) &&
						*integer->pValue >= integer->min && *integer->pValue <= integer->max) {
						goto Finish;
					}
					break;
					/* reject value */
...
MenuCreateInteger (int x, int y, int w, const char *text, int wEdit, int *pValue, int min, int max)
{
	...
	assert (NULL != integer);
	MenuSetItem (&integer->item, MIT_Integer, x, y, w, CELL_H, MenuIntegerFocus, IntegerEventLoop,
				 MenuIntegerMouse, MenuIntegerPoll);
	/* set label */
	integer->text = text;
	integer->lSprite =
		CreateTextSprite (text, (x + 1) * BASE_X, (y + 1) * BASE_Y, (w - wEdit - 2) * BASE_X,
						  (CELL_H - 2) * BASE_Y, FF_TEXT_NO_FOCUS, SPM_MAPPED);
	/* create work buffer */
	...
	/* create editor */
	integer->eSprite =
		CreateTextSprite (integer->work, (x + w - wEdit + 1) * BASE_X, (y + 2) * BASE_Y,
						  (wEdit - 2) * BASE_X, (CELL_H - 4) * BASE_Y, FF_VALUE_NO_SELECT,
						  SPM_MAPPED);
	/* graphics */
