 ...
 *
 * $Id: x11_event.c,v 1.22 2006/02/24 21:57:55 fzago Exp $
 *
 * Program XBLAST
...
static void HandleAsciiKey (XKeyEvent * xkey);
static XBBool HandleChatKey (XKeyEvent * xkey);
	...
	for (i = 1; i < *nelem; i++) {
		if (table[i - 1].key == table[i].key) {
			GUI_ErrorMessage ("Multiple bindings for key \"%s\".",
							  XKeysymToString (XKeycodeToKeysym (dpy, table[i].key, 0)));
			break;
		}
	}
	/* that's all */
 ...
 */
static XBBool
HandleChatKey (XKeyEvent * xkey)
{
	GameKeyEvent *key;
	KeyCode keyCode;
	keyCode = xkey->keycode;
	key = bsearch (&keyCode, keyChatTable, numKeyChat, sizeof (GameKeyEvent), CompareKeyCode);
	if (key != NULL && Chat_isListening ()) {
...
HandleAsciiKey (XKeyEvent * xkey)
{
	...
	if (len == 1) {
		...
		else if (iscntrl ((unsigned char)buf[0])) {
			/* control key */
...
HandleX11Event (XEvent * xev)
{
	...
	case KeyPress:
		switch (keyboardMode) {
		case KB_CHAT:
			(void)HandleChatKey (&xev->xkey);
			break;
		case KB_MENU:
			HandleMenuKey (&xev->xkey);
			break;
		case KB_XBLAST:
			HandleXBlastKey (&xev->xkey);
			break;
		case KB_ASCII:
			HandleAsciiKey (&xev->xkey);
			break;
		case KB_KEYSYM:
			HandleKeysymKey (&xev->xkey);
			break;
		default:
			break;
		}
	...
	case ButtonPress:
		...
		case Button1:
			QueueEventPos (XBE_MOUSE_1, xev->xbutton.x / BASE_X, xev->xbutton.y / BASE_Y);
			break;
		case Button2:
			QueueEventPos (XBE_MOUSE_3, xev->xbutton.x / BASE_X, xev->xbutton.y / BASE_Y);
			break;
		case Button3:
			QueueEventPos (XBE_MOUSE_2, xev->xbutton.x / BASE_X, xev->xbutton.y / BASE_Y);
			break;
		default:
			break;
		}
	...
	case ButtonRelease:
		switch (xev->xbutton.button) {
		case Button1:
			QueueEventPos (XBE_RMOUSE_1, xev->xbutton.x / BASE_X, xev->xbutton.y / BASE_Y);
			break;
		case Button2:
			QueueEventPos (XBE_RMOUSE_3, xev->xbutton.x / BASE_X, xev->xbutton.y / BASE_Y);
			break;
		case Button3:
			QueueEventPos (XBE_RMOUSE_2, xev->xbutton.x / BASE_X, xev->xbutton.y / BASE_Y);
			break;
		default:
			break;
		}
