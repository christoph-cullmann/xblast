 ...
 *
 * $Id: com_newgame.c,v 1.14 2005/01/15 12:40:38 lodott Exp $
 *
 * Program XBLAST
 ...
 */
static void
ReceiveNewGame (XBCommBrowse *bComm, const XBBrowseTele *tele, const char *host, unsigned short port)
{
  assert (NULL != bComm);
  assert (NULL != tele);
  assert (NULL != host);
  switch (tele->type) {
  case XBBT_NewGameOK: HandleReply ((XBCommNewGame *) bComm, &tele->newGameOK, host); break;
  default:  Dbg_Newgame("ignoring invalid response\n"); break;
  }
} /* ReceiveNewGame */
...
EventNewGame (XBCommBrowse *bComm, XBBrowseEvent ev)
{
  ...
  case XBBE_Dgram: Dbg_Newgame("received invalid datagram, ignoring\n"); break;
  case XBBE_Browse: Dbg_Newgame("received invalid browse, ignoring\n"); break;
  case XBBE_Write: Dbg_Newgame("new data waits to be sent\n"); break;
  case XBBE_Close: Dbg_Newgame("browse shutdown complete\n"); break;
  default: Dbg_Newgame("unknown browse event, ignoring!\n");
  }
  return XBFalse;
} /* EventNewGame */
 ...
 */
XBComm *
NewGame_CreateComm (const char *addrDevice, unsigned short port, const char *addrBroadcast, const CFGGameHost *cfg, const CFGGameSetup *setup)
{
  XBSocket      *pSocket;
  XBCommNewGame *cComm;
  ...
  Dbg_Newgame("created udp socket on %s:%u to central %s:%u\n", Socket_HostName(pSocket, XBFalse), Socket_HostPort(pSocket, XBFalse), Socket_HostName(pSocket, XBTrue), Socket_HostPort(pSocket, XBTrue));
  /* create communication data structure */
  cComm = calloc (1, sizeof (*cComm));
  assert (NULL != cComm);
  /* set values */
  Browse_CommInit (&cComm->browse, COMM_NewGame, pSocket, ReceiveNewGame, EventNewGame, DeleteNewGame);
  cComm->port           = port;
  cComm->serial         = 0;
  cComm->gameID         = -1;
