 ...
 *
 * $Id: com_reply.c,v 1.5 2004/10/30 21:18:47 lodott Exp $
 *
 * Program XBLAST
 ...
 */
static void
HandleQuery (XBCommReply *rComm, const XBBrowseTeleQuery *query, const char *host, unsigned short port)
{
 ...
 */
static void
ReceiveReply (XBCommBrowse *bComm, const XBBrowseTele *tele, const char *host, unsigned short port)
{
  assert (NULL != bComm);
  assert (NULL != tele);
  assert (NULL != host);
  switch (tele->type) {
  case XBBT_Query: HandleQuery ((XBCommReply *) bComm, &tele->query, host, port); break;
  default: Dbg_Reply("ignoring invalid query from %s:%u\n", host, port); break;
  }
} /* ReceiveReply */
...
EventReply (XBCommBrowse *bComm, XBBrowseEvent ev)
{
  ...
  switch (ev) {
  case XBBE_Wait: Dbg_Reply("all data sent\n"); break;
  case XBBE_Dgram: Dbg_Reply("received invalid datagram, ignoring\n"); break;
  case XBBE_Browse: Dbg_Reply("received invalid browse, ignoring\n"); break;
  case XBBE_Write: Dbg_Reply("new data waits to be sent\n"); break;
  case XBBE_Close: Dbg_Reply("browse shutdown complete\n"); break;
