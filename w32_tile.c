 ...
 *
 * $Id: w32_tile.c,v 1.6 2004/08/04 08:08:39 iskywalker Exp $
 *
 * Program XBLAST 
...
InitTiles (void)
{
  ...
  /* init led tiles  */
  for (i=0; i<2; i++) {
    pixLeds[i] = ReadCchPixmap (imgPathScore, imgFileScoreLed[i], COLOR_BLACK, COLOR_LIGHT_GOLDENROD, COLOR_SPRING_GREEN);
  }
  // XBCC
  ...
  for (i=0; i < SBDead ; i++) {
    pixScore[i] = ReadCchPixmap (imgPathScore, imgFileScoreTile[i], COLOR_BLACK, COLOR_LIGHT_GOLDENROD, COLOR_SADDLE_BROWN);
  }
  /* init drawing lists */
...
GUI_DrawTimeLed (int x, int block)
{
  ...
  oldDst = SelectObject (hdcDst, pix);
#ifdef SMPF
  BitBlt (hdcDst, x*LED_WIDTH, MAZE_H*BLOCK_HEIGHT + STAT_HEIGHT * 2, LED_WIDTH, LED_HEIGHT, hdcSrc, 0, 0, SRCCOPY);
#else
  BitBlt (hdcDst, x*LED_WIDTH, MAZE_H*BLOCK_HEIGHT + STAT_HEIGHT, LED_WIDTH, LED_HEIGHT, hdcSrc, 0, 0, SRCCOPY);
#endif
  SelectObject (hdcDst, oldDst);
  SelectObject (hdcSrc, oldSrc);
} /* GUI_DrawTimeLed  */
