 ...
 *
 * $Id: player.c,v 1.30 2005/01/11 22:44:41 lodott Exp $
 *
 * Program XBLAST 
...
static const char *permHealthInfo[MAX_ILL] = {
  ...
  "Permanent reverse(2) controls",
  "Permanent random teleporting",
};
static const char *initHealthInfo[MAX_ILL] = {
  NULL,
  "Initial random bombing",
  "Initial slowdown",
  "Initial running",
  "Initial mini bombs",
  "No bombs while healthy",
  ...
  "Initial reverse controls",
  "Initial reverse(2) controls",
  "Initial random teleporting",
};
static const char *reviveHealthInfo[MAX_ILL] = {
  NULL,
  "Revived with random bombing",
  "Revived with slowdown",
  "Revived with running",
  "Revived with mini bombs",
  "Revived with bombs while healthy",
  "Revived with invisibility",
  "Revived with malfunctions",
  "Revived with reverse controls",
  "Revived with reverse(2) controls",
  "Revived with random teleporting",
};
...
ConfigLevelPlayers (const DBSection *section, XBBool allowRandomPos, unsigned gameMode)
{
  ...
  for (i = 0; i < numPlayer; i ++) {
    ...
    ps->num_extras     = (LF_Snipe     == iniplayerflags) ? 1000:((LF_Morph  ==iniplayerflags) ? 1000 :0);
    ps->speed=0;
  ...
  case 1:  AddPlayerInfo ("1 bomb");             break;
  default: AddPlayerInfo ("%d bomb", minBombs); break;
  }
  switch (minRange) {
  case 0:  AddPlayerInfo ("No initial range");           break;
  case 1:  AddPlayerInfo ("Only mini bombs");              break;
  default: AddPlayerInfo ("Initial range %d", minRange); break;
  }
  ...
  if (iniplayerflags ==LF_Daleif){
    if(revplayerflags ==RF_Daleif){
      AddPlayerInfo ("Daleif as default");
    } else {
      AddPlayerInfo ("Initital Daleif");
    }
  }else if( (revplayerflags ==RF_Daleif)) {
    AddPlayerInfo ("Revived with Daleif");
  }
  if (iniplayerflags ==LF_RC) {
    if(revplayerflags ==RF_RC) {
      AddPlayerInfo ("Remote control as default");
    } else {
      AddPlayerInfo ("Initital remote control");
    }
  } else if( (revplayerflags ==RF_RC)) {
    AddPlayerInfo ("Revived with remote control");
  }
  if (iniplayerflags ==LF_Jump){
    if(revplayerflags ==RF_Jump) {
      AddPlayerInfo ("Jump as default");
    } else {
      AddPlayerInfo ("Initital Jump");
    }
  } else if( (revplayerflags ==RF_Jump)) {
    AddPlayerInfo ("Revived with Jump");
  }
  if (iniplayerflags ==LF_Airpump) {
    if(revplayerflags ==RF_Airpump) {
      AddPlayerInfo ("Airpump as default");
    } else {
      AddPlayerInfo ("Initital airpump");
    }
  } else if(revplayerflags ==RF_Airpump) {
    AddPlayerInfo ("Revived with airpump");
  }
  if (iniplayerflags ==LF_Cloak) {
    if (revplayerflags ==RF_Cloak) {
      AddPlayerInfo ("Cloak as default");
    } else {
      AddPlayerInfo ("Initital cloak");
    }
  } else if (revplayerflags ==RF_Cloak) {
    AddPlayerInfo ("Revived with cloak");
  }
  if (iniplayerflags &LF_Kick) {
    if(revplayerflags ==RF_Kick) {
      AddPlayerInfo ("Initital kick");
    } else {
      AddPlayerInfo ("Initital kick");
    }
  } else if (revplayerflags ==RF_Kick) {
    AddPlayerInfo ("Revived with kick");
  }
  if(iniplayerflags ==LF_Morph) {
    if(revplayerflags ==RF_Morph) {
      AddPlayerInfo ("Morphing as default");
    } else {
      AddPlayerInfo ("Initital morphing");
    }
  } else if (revplayerflags ==RF_Morph) {
    AddPlayerInfo ("Revived with morphing");
  }
  if(iniplayerflags ==LF_Through) {
    if(revplayerflags ==RF_Through) {
      AddPlayerInfo ("Throughing as default");
    } else {
      AddPlayerInfo ("Initital throughing");
    }
  } else if (revplayerflags ==RF_Through) {
    AddPlayerInfo ("Revived with throughing");
  }
  /* fprintf(stderr," suck %i %i %i %i %i %i %i %i %i %i\n",iniplayerflags,LF_Suck,revplayerflags,RF_Suck, ps->iniextra_flags,initFlags,(!((1<<29)|(1<<30))), IF_Kick, (1<<29),(!(1<<29))); */
  if (iniplayerflags ==LF_Suck){
    if(revplayerflags ==RF_Suck) {
      AddPlayerInfo ("Sucker as default");
    } else {
      AddPlayerInfo ("Initital sucker");
    }
  } else if (revplayerflags ==RF_Suck) {
    AddPlayerInfo ("Revived with sucker");
  }
  /** Skywalker 
      switch (iniplayerflags ==LF_Daleif && (revplayerflags ==RF_Daleif)playerflags == IF_Snipe) {
      case IF_Snipe: AddPlayerInfo ("Sniping as default");   break;
      case LF_Snipe: AddPlayerInfo ("Initital sniping");     break;
      case RF_Snipe: AddPlayerInfo ("Revived with sniping"); break;
      default:         break;
      } 
      switch (iniplayerflags ==LF_Daleif && (revplayerflags ==RF_Daleif)playerflags == IF_RC) {
      case IF_RC: AddPlayerInfo ("Remote control as default");   break;
      case LF_RC: AddPlayerInfo ("Initital remote control");     break;
      case RF_RC: AddPlayerInfo ("Revived with remote control"); break;
      default:         break;
      }
      switch (iniplayerflags ==LF_Daleif && (revplayerflags ==RF_Daleif)playerflags == IF_Teleport) {
      case IF_Teleport: AddPlayerInfo ("Teleport as default");   break;
      case LF_Teleport: AddPlayerInfo ("Initital teleport");     break;
      case RF_Teleport: AddPlayerInfo ("Revived with teleport"); break;
      default:         break;
      }
      switch (iniplayerflags ==LF_Daleif && (revplayerflags ==RF_Daleif)playerflags == IF_Frogger) {
      case IF_Frogger: AddPlayerInfo ("Frogger as default");   break;
      case LF_Frogger: AddPlayerInfo ("Initital frogger");     break;
      case RF_Frogger: AddPlayerInfo ("Revived with frogger"); break;
      default:         break;
      }
      switch (iniplayerflags ==LF_Daleif && (revplayerflags ==RF_Daleif)playerflags == IF_Stop) {
      case IF_Stop: AddPlayerInfo ("Stop as default");   break;
      case LF_Stop: AddPlayerInfo ("Initital stop");     break;
      case RF_Stop: AddPlayerInfo ("Revived with stop"); break;
      default:         break;
      }
  
      switch (iniplayerflags ==LF_Daleif && (revplayerflags ==RF_Daleif)playerflags == IF_Phantom) {
      case IF_Phantom: AddPlayerInfo ("Phantom as default");   break;
      case LF_Phantom: AddPlayerInfo ("Initital Phantom");     break;
      case RF_Phantom: AddPlayerInfo ("Revived with Phantom"); break;
      default:         break;
      }
      switch (iniplayerflags ==LF_Electrify && (revplayerflags ==RF_Electrify)_Electrify) {
      case IF_Electrify: AddPlayerInfo ("Electrify as default");   break;
      case LF_Electrify: AddPlayerInfo ("Initital Electrify");     break;
      case RF_Electrify: AddPlayerInfo ("Revived with Electrify"); break;
      default:         break;
      }*/
  /* that's all folks */
} /* ConfigLevelPlayers */
...
InitPlayerStat (BMPlayer *ps, int player, int ctrl, XBPlayerTeam team, int PID, XBBool local) // XBCC
{
  ...
  fprintf(stderr," new sprite player %i %i\n",((ps->sprite)->player).player,ps->id);
...
InitPlayerMessages (PlayerStrings *str, const CFGPlayer *cfgPlayer)
{
  ...
  if (NULL != cfgPlayer->messages.msgWinGame) {
    ...
    str->wingame = DupString ("CONGRATULATIONS!");
  }
...
InitPlayers (XBPlayerHost host, const CFGGame *cfgGame, const CFGPlayer *cfgPlayer)
{
 ...
 if (host == cfgGame->players.host[i]){
   ps->localDisplay=i;
 }else {
   ps->localDisplay=-1;
 }
    if (cfgGame->players.team[i] == XBPT_None) {
      local=(host == cfgGame->players.host[i]);
    } else {
      local=(host == cfgGame->players.host[i]);
      for (j = 0; j < cfgGame->players.num; j ++) {
	if (host == cfgGame->players.host[j]) {
...
DropBomb (BMPlayer *ps, int type)
{
  if ( (ps->bombs !=0) && (ps->sniping != 1) && /* skywalker / koen */
    ...
    if (ps->lives >0) {
      if(ps->choice_bomb_type != NUM_BMT) type=ps->choice_bomb_type;
...
{
  if ( !(flag && CheckMazeGhost(ps->ghost,mazex, mazey-1))||(!((ps->phantom)?CheckMazePhantomWall(mazex, mazey-1):CheckMaze(mazex, mazey-1))&&(mazey > 1))) {
    ps->y -= STEP_VERT;
    if(ps->y<0) ps->y = PIXH-BLOCK_HEIGHT*2; // 02-05-2002
  ...
  if (CheckBomb (mazex, mazey) 
    ...
    if (ps->kick) {
      ...
      /* removed by Galatius Sky */ //  MoveBomb (mazex, mazey, GoUp);
...
WalkLeft (BMPlayer *ps, int flag, int mazex, int mazey)
{
  if ( !(flag && CheckMazeGhost(ps->ghost,mazex -1, mazey) )||
       (!((ps->phantom)?CheckMazePhantomWall(mazex-1, mazey):CheckMaze(mazex-1, mazey))&&(mazex > 1))) {
    ps->x -= STEP_HORI;
    if(ps->x<0) ps->x = PIXW; // 02-05-2002
  ...
  if ( CheckBomb(mazex, mazey) 
    ...
    if (ps->kick) {
      SND_Play (SND_KICK, ps->x / (PIXW / MAX_SOUND_POSITION));
      /* removed by Galatius Sky */     //     MoveBomb (mazex, mazey, GoLeft);
      /* added by Galatius */
      switch(ps->daleif &  GameRandomNumber(2)){
      case 0:
...
WalkDown (BMPlayer *ps, int flag, int mazex, int mazey)
{
  if ( !(flag && CheckMazeGhost(ps->ghost,mazex, mazey+1) )||
       (!((ps->phantom)?CheckMazePhantomWall(mazex, mazey+1):CheckMaze(mazex, mazey+1))&&(mazey < (MAZE_H-2)))) {
    ps->y += STEP_VERT;
    if(ps->y>=(PIXH-BLOCK_HEIGHT*2)) ps->y = 0; // 02-05-2002
  ...
  if ( CheckBomb(mazex, mazey+1)
    ...
    if (ps->kick) {
      SND_Play (SND_KICK, ps->x / (PIXW / MAX_SOUND_POSITION));
      /* removed by Galatius Sky */   //  MoveBomb(mazex, mazey+1,	GoDown);/* added by Galatius */
      switch(ps->daleif &  GameRandomNumber(2)){
      case 0:
...
{
  if ( !(flag && CheckMazeGhost(ps->ghost,mazex +1, mazey))||
       (!((ps->phantom)?CheckMazePhantomWall(mazex+1, mazey):CheckMaze(mazex+1, mazey))&&(mazex < (MAZE_W-2)))) {
    ps->x += STEP_HORI;
    if(ps->x>=PIXW-BLOCK_WIDTH) ps->x = 0; // 02-05-2002
    mazex = ps->x/BLOCK_WIDTH;
    if (ps->illness != IllReverse) {
      if (ps->illness == IllReverse2) {
  ...
  if ( CheckBomb(mazex+1, mazey)
    ...
    if (ps->kick) {
      SND_Play (SND_KICK, ps->x / (PIXW / MAX_SOUND_POSITION));
      /* removed by Galatius Sky */    //   MoveBomb(mazex+1, mazey, GoRight);
      /* added by Galatius */
      switch(ps->daleif &  GameRandomNumber(2)){
      case 0:
...
TeleportPlayer (BMPlayer *ps, int mazeX, int mazeY) 
{
  ...
  /*
    for (i = 0; i < NUM_TELE_TRIES; i ++) {
    newMazeX = GameRandomNumber (MAZE_W);
    newMazeY = GameRandomNumber (MAZE_H);
    if (! CheckMaze (newMazeX, newMazeY) && 
  ...
  if(n>0) {
    ...
    newMazeY=i / MAZE_W;
    /*to go through walls 
      if (((ps->phantom)?(!CheckMazePhantomWall2(newMazeX,newMazeY)):(!CheckMaze(newMazeX,newMazeY))) */
    if (((ps->phantom)?(!CheckMazePhantomWall(newMazeX,newMazeY)):(!CheckMaze(newMazeX,newMazeY)))
	&& ((mazeX != newMazeX) || (mazeY != newMazeY))) {
      SND_Play (SND_TELE1, ps->x / (PIXW / MAX_SOUND_POSITION));
      ps->x      =  newMazeX    * BLOCK_WIDTH;
      ps->y      = (newMazeY-1) * BLOCK_HEIGHT;
...
DoWalk (BMPlayer *ps, int gameTime)
{
  ...
  if (ps->illness != IllSlow || 0 == gameTime % 2) {
	...
	if (ps->d_ist != GoStop) {
      ...
      /* random teleporting */
      if ( (ps->illness == IllTeleport) && 
	   (0 == GameRandomNumber (32) ) ) {
	...
	switch (ps->d_ist) {
	case GoStop:  WalkStop  (ps, flag, mazeX, mazeY); break;
	case GoLeft:  WalkLeft  (ps, flag, mazeX, mazeY); break;
	case GoRight: WalkRight (ps, flag, mazeX, mazeY); break;
	case GoDown:  WalkDown  (ps, flag, mazeX, mazeY); break;
	case GoUp:    WalkUp    (ps, flag, mazeX, mazeY); break;
	default:      break;
	}
      }
	...
	case BTSick:
	  ...
	  if (ps->illness == IllReverse) {
	    switch (ps->d_ist) {
	    case GoDown:  ps->d_ist = GoUp;    break;
	    case GoUp:    ps->d_ist = GoDown;  break;
	    case GoLeft:  ps->d_ist = GoRight; break;
	    case GoRight: ps->d_ist = GoLeft;  break;
	    default:      break;
	    }
	  }
	  if (ps->illness == IllReverse2) {
	    switch (ps->d_ist) {
	    case GoDown:  ps->d_ist = GoLeft;    break;
	    case GoUp:    ps->d_ist = GoRight;  break;
	    case GoLeft:  ps->d_ist = GoUp; break;
	    case GoRight: ps->d_ist = GoDown;  break;
	    default:      break;	    }
	  }
	  break;
...
/* Added by "Belgium Guys" */  
  if (ps->through) { ps->through--; }
  if(ps->ghost) { ps->ghost--;
/* Written by VVL */     
 // if (ps->through) { ps->through--; }
...
/* Written by VVL */ /**/
/* Written by VVL */ /* public function Electrify_other_players */
/* Written by VVL */ /**/
/* Written by VVL */ int
/* Written by VVL */ ElectrifyOtherPlayers (int nplayer)
     /* Written by VVL */ {
  /* Written by VVL */   int player;
  /* Written by VVL */   int count=0;
  /* Written by VVL */ 
  /* Written by VVL */   for (player = 0; player <numPlayer; player ++) {
    /* Written by VVL */     if (nplayer!=player) {
      /* Written by VVL */      if(player_stat[player].lives) {
	/* Written by VVL */         if( (ABS(player_stat[player].x-player_stat[nplayer].x)<(BLOCK_WIDTH*3/4))
					 /* Written by VVL */             && (ABS(player_stat[player].y-player_stat[nplayer].y)<(BLOCK_HEIGHT*3/4))) {
	  /* Written by VVL */           player_stat[player].dying=DEAD_TIME;
	  /* Written by VVL */           player_stat[player].electrify=0;
	  /* Written by VVL */           count++;
	  /* Written by VVL */         }
	/* Written by VVL */      }
      /* Written by VVL */     }
 /* Written by VVL */   }
  /* Written by VVL */   return (count>0);
  /* Written by VVL */ }
	...
	if (ps1->lives && ps2->lives){
	  /* infection with "evil grail" virus */
 	  ...
 	  if(ps1->evilill && (! ps2->invincible)){
 	    ...
 	    if (ps1->cloaking > 0) ps1->cloaking = -(ps1->cloaking + BONUSEVIL);
 	    else ps1->cloaking -= BONUSEVIL;
	    ps1->revive += 1;
	    ...
	    if (ps2->cloaking > 0) ps2->cloaking = -(ps2->cloaking + BONUSEVIL);
	    else ps2->cloaking -= BONUSEVIL;
	    ps2->revive += 1;
	  ...
	  /* Written by Amilhastre */ 	  if ((ps2->lives) && (ps1->revive)){
	    /* Written by Amilhastre */ 	    equipe = -1;
	    /* Written by Amilhastre */ 	    team_alive = XBFalse;
	    /* Written by Amilhastre */ 	    for (i = 0, ptr = player_stat; i<numPlayer; i++, ptr++) {
	      /* Written by Amilhastre */ 	      if (ptr->team == ps1->team) {
		/* Written by Amilhastre */ 		team_alive |= (ptr->lives != 0);
		/* Written by Amilhastre */ 		equipe++;
		/* Written by Amilhastre */ 	      }
	      /* Written by Amilhastre */ 	    }
	    /* Written by Amilhastre */ 	    
	    /* Written by Amilhastre */ 	    if (!team_alive) (*active_player) ++;
	    /* Written by Amilhastre */ 	    ps1->lives = 1;
	    /* Written by Amilhastre */ 	    ps1->revive--;
	    /* Written by Amilhastre */ 	  }
	  /* Written by Amilhastre */ 
	  /* Written by Amilhastre */ 
	  /* Written by Amilhastre */ 	  if ((ps1->lives) && (ps2->revive)){
	    /* Written by Amilhastre */ 	    equipe = -1;
	    /* Written by Amilhastre */ 	    team_alive = XBFalse;
	    /* Written by Amilhastre */ 	    for (i = 0, ptr = player_stat; i<numPlayer; i++, ptr++) {
	      /* Written by Amilhastre */ 	      if (ptr->team == ps2->team) {
		/* Written by Amilhastre */ 		team_alive |= (ptr->lives != 0);
		/* Written by Amilhastre */ 		equipe++;
		/* Written by Amilhastre */ 	      }
	      /* Written by Amilhastre */ 	    }
	    /* Written by Amilhastre */ 	    
	    /* Written by Amilhastre */ 	    if (!team_alive) (*active_player) ++;
	    /* Written by Amilhastre */ 	    ps2->lives = 1;
	    /* Written by Amilhastre */ 	    ps2->revive--;
	    /* Written by Amilhastre */ 	     
	    /* Written by Amilhastre */ 	  }
	}
      }
...
void SetMsgLaola  (int player){
 SetMessage (p_string[player].laola, XBFalse);

}
void SetMsgLoser  (int player){
  ...
  if(numPlayer==2)
      ...
      if(count>2) 
	{
	  ...
	  for (player = 0; player<count-1; player++ )
	    {
	      ((player_stat[D[player]].sprite)->player).player=((player_stat[D[player+1]].sprite)->player).player;
      ...
      else
	{
	  if(count==2)
	    {
	      RR=((player_stat[D[0]].sprite)->player).player;
	      ((player_stat[D[0]].sprite)->player).player=((player_stat[D[1]].sprite)->player).player;
...
/*Skywalker */
 ...
 /* Added by "Belgium Guys" */ /**/
 /* Added by "Belgium Guys" */ /* public function steal_bombs_other_players */
 /* Added by "Belgium Guys" */ /**/
/* Added by "Belgium Guys" */ int
/* Added by "Belgium Guys" */ StealBombsOtherPlayers (int team)
     /* Added by "Belgium Guys" */ {
  /* Added by "Belgium Guys" */   int count = 0;
  /* Added by "Belgium Guys" */   int player;
  /* Added by "Belgium Guys" */ 
  /* Added by "Belgium Guys" */   for (player = 0; player <numPlayer; player ++) {
    /* Added by "Belgium Guys" */     if (player_stat[player].team != team)
      /* Added by "Belgium Guys" */     {
      /* Added by "Belgium Guys" */ 	if(player_stat[player].bombs > 0) {
	/* Added by "Belgium Guys" */ 		player_stat[player].bombs--;
	/* Added by "Belgium Guys" */ 		count++; }
 /* Added by "Belgium Guys" */     }
    /* Added by "Belgium Guys" */   }
  /* Added by "Belgium Guys" */   return count;
  /* Added by "Belgium Guys" */ }
/* Added by "Belgium Guys" */ /**/
/* Added by "Belgium Guys" */ /* public function steal_range_other_players */
/* Added by "Belgium Guys" */ /**/
/* Added by "Belgium Guys" */ int
/* Added by "Belgium Guys" */ StealRangeOtherPlayers (int team)
     /* Added by "Belgium Guys" */ {
  /* Added by "Belgium Guys" */   int count = 0;
  /* Added by "Belgium Guys" */   int player;
  /* Added by "Belgium Guys" */   for (player = 0; player <numPlayer; player ++) {
    /* Added by "Belgium Guys" */     if (player_stat[player].team != team)
      /* Added by "Belgium Guys" */     {
      /* Added by "Belgium Guys" */         if(player_stat[player].range > 1) {
	/* Added by "Belgium Guys" */                 player_stat[player].range--;
	/* Added by "Belgium Guys" */                 count++; }
      /* Added by "Belgium Guys" */     }
    /* Added by "Belgium Guys" */   }
  /* Added by "Belgium Guys" */   return count;
...
SwapPositionOtherPlayers (int team)
{
  ...
  if(count>1) {
    if(first>-1) { player_stat[first].x=x;player_stat[first].y=y; }
  }
...
RevivePlayer (BMPlayer *ps, int *active_player)
{
  ...
  if (ps->lives == 0) {
    ...
    DistributeExtras (ps->bombs - minBombs, ps->range - minRange, ps->num_extras, ps->special_bombs);
  ...
  if (RF_Revive == playerflags) {
    /* Written by Amilhastre */     ps->revive = 1;
    /* Written by Amilhastre */   }
  ps->choice_bomb_type = NUM_BMT;
  if (ps->local )ResetMessage();
...
{ 
  ...
  for (p = 0; p <numPlayer; p ++) {
	...
	if (player_stat[player].morphed) {
	  ...
	  if (player_stat[player].farted){ /* (galatius) */

	    DoWalk(player_stat + player, game_time);

	    // if(player_stat[player].sniping==0)
	...
	/* added by Galatius*/	if (player_stat[player].smelly){player_stat[player].smelly--;}
	/* added by Galatius*/	if (player_stat[player].daleifing){player_stat[player].daleifing--;}
	break;
	...
	break;
      ...
      /* Written by Amilhastre */       if (player_stat[player].revive){
	/* Written by Amilhastre */ 	if (player_stat[player].cloaking < 0) {
	  /* Written by Amilhastre */ 	  player_stat[player].cloaking ++;
	  /* Written by Amilhastre */ 	  if (player_stat[player].cloaking & 0x01) {
	    /* Written by Amilhastre */ 	    spm_mode = player_stat[player].disp;
	    /* Written by Amilhastre */ 	  } else {
	      /* Written by Amilhastre */ 	    spm_mode = SPM_UNMAPPED;
	      /* Written by Amilhastre */ 	  }
	  /* Written by Amilhastre */ 	}
	/* Written by Amilhastre */ 	else 
	  /* Written by Amilhastre */ 	  spm_mode = SPM_MAPPED;
	/* Written by Amilhastre */ 	SetSpriteMode (player_stat[player].sprite, spm_mode);
	/* Written by Amilhastre */ 	SetSpriteAnime (player_stat[player].sprite, SpriteZombie);
	/* Written by Amilhastre */ 	
	/* Written by Amilhastre */       }
      /* Added by VVL (Chat) 12/11/99 : Begin */
      /* if ((player_stat[player].chatmode == 1) &&(Original&(1<<player_stat[player].id))) {
	set_get(player_stat[player].chatstring,TRUE);
	}*/
      /* Added by VVL (Chat) 12/11/99 : End */
      /* Written by Amilhastre */ }
...
CheckPlayerHit (void)
{
  ...
  for (player = 0; player < numPlayer; player ++) {
    ...
    if ( 0 != player_stat[player].lives	     && 
	 ...
	 0 == player_stat[player].dying      && 
	 CheckExplosion (gridx, gridy) ) {
      player_stat[player].dying = DEAD_TIME;
    }
  }
} /* CheckPlayerHit */
