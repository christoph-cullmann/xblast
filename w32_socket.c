 ...
 *
 * $Id: w32_socket.c,v 1.8 2005/01/15 17:41:43 iskywalker Exp $
 *
 * Program XBLAST
...
#ifndef WMS
    Dbg_Socket("failed to connect socket fd=%u to %s:%u, err=%u\n", Socket_HostName(pSocket, XBTrue), Socket_HostPort(pSocket, XBTrue), WSA_GetLastError());
#endif
    return XBFalse;
  }
...
#ifndef WMS
    Dbg_Socket("failed to listen on socket fd=%u (%s:%u), err=%u\n", pSocket->fd, Socket_HostName (pSocket, XBFalse), Socket_HostPort (pSocket, XBFalse), WSA_GetLastError());
#endif
    return XBFalse;
  }
...
Socket_GetInterfaces (size_t *num)
{
  ...
  while (SOCKET_ERROR == WSAIoctl (fd, SIO_GET_INTERFACE_LIST, NULL, 0, info, iLen*sizeof(XB_INTERFACE_INFO), &len, NULL, NULL) ) {
  ...
  for (nr = 0; nr < len; nr ++) {
    ...
    if (pAddrInet->sin_family == AF_INET &&
        pAddrInet->sin_addr.s_addr != INADDR_ANY &&
        (iFlags & (IFF_UP|IFF_BROADCAST)) ) {
