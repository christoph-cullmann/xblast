 ...
 *
 * $Id: mi_color.c,v 1.3 2004/05/14 10:00:35 alfie Exp $
 * 
 * Program XBLAST 
...
MenuCreateColor (int x, int y, int w, const char *text, XBColor *pColor, XBRgbValue *pRgb)
{
  ...
  assert (color != NULL);
  MenuSetItem (&color->item, MIT_Color, x, y, w, CELL_H, MenuColorFocus, MenuColorSelect, MenuColorMouse, MenuColorPoll);
  /* set label */
  color->text    = text;
  color->tSprite = CreateTextSprite (text, (x + 3) * BASE_X, (y + 1) * BASE_Y, (w - CELL_W - 4) * BASE_X, (CELL_H - 2) * BASE_Y,
				     FF_COLOR_TEXT_NO_FOCUS, SPM_MAPPED);
  /* get color */
  color->pointer = pColor;
  color->current = *pColor;
  color->pRgb    = pRgb;
  color->index   = colorCount ++;
  color->cSprite = CreateIconSprite ((x + w - CELL_W) * BASE_X, y * BASE_Y, ISA_Color1 + color->index, SPM_MAPPED);
  SetSpriteColor (color->cSprite, color->current);
