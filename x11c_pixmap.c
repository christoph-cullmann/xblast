 ...
 *
 * $Id: x11c_pixmap.c,v 1.9 2006/02/09 21:21:25 fzago Exp $
 *
 * Program XBLAST 
...
InitPixmap (void)
{
	...
	xgcv.fill_style = FillTiled;
	xgcv.tile =
		ReadCchPixmap (imgPathMisc, imgFileTitle, COLOR_BLACK, cfgColor->titleFg,
					   cfgColor->titleBg);
...
GUI_FlushPixmap (XBBool flag)
{
	...
	if (!flag) {
		...
		for (i = 0; i < 4; i++) {
			XFillRectangle (dpy, win, gcFromPix, 0, (PIXH + SCOREH) / 4 * i, PIXW,
							(PIXH + SCOREH) / 4);
		}
	}
	else {
