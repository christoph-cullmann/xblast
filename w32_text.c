 ...
 *
 * $Id: w32_text.c,v 1.3 2004/08/06 01:57:27 iskywalker Exp $
 *
 * Program XBLAST 
...
InitText (void)
{
  ...
  /* create pens */
  for (i = 0; i < NUM_PEN; i ++) {
    hPen[i] = CreatePen (initPen[i].style, initPen[i].width, COLOR_TO_COLORREF (initPen[i].color) );
  }
  /* create brushes */
 ...
 */
static int
DrawAlignedText (const char *text, int len, unsigned flags, int x, int y, int w, HDC hdc_fg, HDC hdc_bg, HGDIOBJ *pOld)
{
  SIZE textSize;
  int  xpos;
  int  dx, dy;
...
GUI_DrawTextbox (const char *text, unsigned flags, const BMRectangle *rect)
{
  ...
  if (text != NULL) {
	...
	right = DrawAlignedText (text + i, 1, flags, rect->x, y2, rect->w, hdc, hdcInv, &old);
      }
    } else {
