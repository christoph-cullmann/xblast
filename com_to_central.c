 ...
 *
 * $Id: com_to_central.c,v 1.5 2004/11/11 14:57:08 lodott Exp $
 *
 * Program XBLAST
...
X2C_CreateComm (const CFGCentralSetup *cfg)
{
  ...
  /* set values */
  Stream_CommInit (&toCentral->stream, COMM_ToCentral, pSocket, HandleTelegram, EventToCentral, DeleteToCentral);
  /* that's all */
...
X2C_QueryPlayerConfig (XBComm *comm)
{
  ...
  Net_SendTelegram (stream->sndQueue, Net_CreateTelegram (XBT_COT_RequestData, XBT_ID_PlayerConfig, 0, NULL, 0) );
...
X2C_SendGameStat (XBComm *comm, int numPlayers, int *PID, int *Score)
{
  ...
  Socket_RegisterWrite (CommSocket (&stream->comm));
  Net_SendTelegram (stream->sndQueue, Net_CreateTelegram (XBT_COT_SendData, XBT_ID_GameStat, 0, tmp, 4+numPlayers*8) );
...
X2C_SendDisconnect (XBComm *comm)
{
  ...
  Socket_RegisterWrite (CommSocket (&stream->comm));
  Net_SendTelegram (stream->sndQueue, Net_CreateTelegram (XBT_COT_Spontaneous, XBT_ID_HostDisconnected, 0, NULL, 0) );
  Net_SendTelegram (stream->sndQueue, Net_CreateTelegram (XBT_COT_Activate, XBT_ID_RequestDisconnect, 0, NULL, 0) );
