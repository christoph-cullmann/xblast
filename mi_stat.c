 ...
 *
 * $Id: mi_stat.c,v 1.5 2004/06/04 11:24:19 iskywalker Exp $
 *
 * Program XBLAST 
...
MenuTablePoll (XBMenuItem *ptr)
{
  ...
  if (table->pollPtr  != NULL &&
      *table->pollPtr != table->pollData) {
...
CreateTableItem (int x, int y, int w, MIC_button func, void *funcData, TablePollFunc pollFunc, const void **pollPtr, int num)
{
  /* create item */
  XBMenuTableItem *table = calloc (1, sizeof (XBMenuTableItem) ); 
  assert (table != NULL);
  MenuSetItem (&table->item, MIT_Table, x, y, w, CELL_H, MenuTableFocus, MenuTableSelect, MenuTableMouse, 
	       (NULL != pollFunc) ? MenuTablePoll : NULL);
  /* cell sprites */
  table->numCells = num;
...
SetTableCell (XBMenuTableItem *table, size_t cell, unsigned align, int x, int w, const char *fmt, ...)
{
  ...
  table->cell[cell].sprite = CreateTextSprite (table->cell[cell].text, BASE_X *(table->item.x + x), BASE_Y * table->item.y,
					       BASE_X * w, (CELL_H/2) * BASE_Y, FF_TABLE_NO_FOCUS | align, SPM_MAPPED);
  /* checks */
  assert (NULL != table->cell[cell].text);
  assert (NULL != table->cell[cell].sprite);
} /* SetTableCell */
...
MenuCreateStatEntry (int x, int y, int w, const XBStatData *data, MIC_button func, void *funcData)
{
  ...
  SetTableCell (table, 2, FF_Right, w - 1 - 10*CELL_W/2, 3*CELL_W/2, "%6.3f",   data->average);
  SetTableCell (table, 3, FF_Right, w - 1 -  7*CELL_W/2,   CELL_W,   "%d",      data->numWon);
  SetTableCell (table, 4, FF_Right, w - 1 -  5*CELL_W/2, 3*CELL_W/2, "%5.2f%%", data->percent);
...
MenuCreateStatHeader (int x, int y, int w, const char *title)
{
  ...
  SetTableCell (table, 1, FF_Boxed|FF_Right, w - 1 - 12*CELL_W/2,   CELL_W,          "Score");
  SetTableCell (table, 2, FF_Boxed|FF_Right, w - 1 - 10*CELL_W/2, 3*CELL_W/2,        "Avg. Score");
  SetTableCell (table, 3, FF_Boxed|FF_Right, w - 1 -  7*CELL_W/2,   CELL_W,          "# Won");
  SetTableCell (table, 4, FF_Boxed|FF_Right, w - 1 -  5*CELL_W/2, 3*CELL_W/2,        "%% Won");
  SetTableCell (table, 5, FF_Boxed|FF_Right, w - 1 -  2*CELL_W/2,   CELL_W,          "# Total");
  /* --- */
  return &table->item;
} /* CreateMenuStatHeader */

/*
 * XBCC
 */
XBMenuItem *
MenuCreateCentralEntry (int x, int y, int w, const XBCentralData *data, MIC_button func, void *funcData)
{
  XBMenuTableItem *table = CreateTableItem (x, y, w, func, funcData, NULL, NULL, 6);
  /* set cells ... */
  assert (data != NULL);
  ...
  SetTableCell (table, 1, FF_Right, w - 1 - 12*CELL_W/2, 3*CELL_W/2, "%6.1f",   data->score);
...
MenuCreateCentralHeader (int x, int y, int w, const char *title)
{
  ...
  /* set cells ... */
  SetTableCell (table, 0, FF_Boxed|FF_Left,  1,                   w - 6*CELL_W - 2,  title);
  SetTableCell (table, 1, FF_Boxed|FF_Right, w - 1 - 12*CELL_W/2, 3*CELL_W/2,        "Rating");
  SetTableCell (table, 2, FF_Boxed|FF_Right, w - 1 -  9*CELL_W/2,   CELL_W,          "Rank");
  SetTableCell (table, 3, FF_Boxed|FF_Right, w - 1 -  7*CELL_W/2,   CELL_W,          "# Won");
  SetTableCell (table, 4, FF_Boxed|FF_Right, w - 1 -  5*CELL_W/2, 3*CELL_W/2,        "%% Won");
  SetTableCell (table, 5, FF_Boxed|FF_Right, w - 1 -  2*CELL_W/2,   CELL_W,          "# Total");
  /* --- */
 ...
 */
XBMenuItem *
MenuCreateInfoEntry (int x, int y, int w, const XBCentralInfo *data, MIC_button func, void *funcData)
{
...
MenuCreateInfoHeader (int x, int y, int w, const char *title)
{
  ...
  /* set cells ... */
  SetTableCell (table, 0, FF_Boxed|FF_Left,  1,                   w - 6*CELL_W - 2,  "Parameter");
  SetTableCell (table, 1, FF_Boxed|FF_Left, w - 1 - 12*CELL_W/2, 12*CELL_W/2,        "Value");
 /* --- */
  return &table->item;
} /* CreateMenuStatHeader */
 ...
 */
XBMenuItem *
MenuCreateDemoEntry (int x, int y, int w, const CFGDemoEntry *data, MIC_button func, void *funcData)
{
  ...
  SetTableCell (table, 1, FF_Left,   1 + 5*CELL_W/2,    (w - 4*CELL_W - 2), GetLevelNameByAtom (data->level) );
  SetTableCell (table, 2, FF_Center, w - 1 - 3*CELL_W/2, 3*CELL_W/2,        "%d", data->numPlayers);
  /* --- */
  return &table->item;
} /* CreateMenuTable */
...
MenuCreateDemoHeader (int x, int y, int w)
{
  ...
  /* set cells */
  SetTableCell (table, 0, FF_Boxed|FF_Left,   1,                  5*CELL_W/2,        "Date & Time");
  SetTableCell (table, 1, FF_Boxed|FF_Left,   1 + 5*CELL_W/2,    (w - 4*CELL_W - 2), "Level");
  SetTableCell (table, 2, FF_Boxed|FF_Center, w - 1 - 3*CELL_W/2, 3*CELL_W/2,        "#players");
  /* --- */
  return &table->item;
} /* CreateMenuTable */
...
{
  XBMenuTableItem *table = CreateTableItem (x, y, w, func, game, PollGameEntry, (const void **) game, 7);
  /* set cells */
...
MenuCreateGameHeader (int x, int y, int w)
{
  ...
  /* set cells */
  SetTableCell (table, 0, FF_Boxed|FF_Left,   1,                  w-2 - 16*CELL_W/2, "Game");
  SetTableCell (table, 1, FF_Boxed|FF_Center, w - 1 -16*CELL_W/2,      	11*CELL_W/4, "Host");
  SetTableCell (table, 2, FF_Boxed|FF_Center, w - 1 -21*CELL_W/4,      	 2*CELL_W/2, "Ping");
  SetTableCell (table, 3, FF_Boxed|FF_Center, w - 1 -17*CELL_W/4,      	 5*CELL_W/4, "Version");
  SetTableCell (table, 4, FF_Boxed|FF_Center, w - 1 - 6*CELL_W/2,      	 2*CELL_W/2, "#lives");
  SetTableCell (table, 5, FF_Boxed|FF_Center, w - 1 - 4*CELL_W/2,      	 2*CELL_W/2, "#wins");
  SetTableCell (table, 6, FF_Boxed|FF_Center, w - 1 - 2*CELL_W/2,      	 2*CELL_W/2, "FPS");
  /* --- */
  return &table->item;
} /* CreateMenuTable */
