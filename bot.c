...
/* Variables déclarées dans bomb.c */
extern Explosion *bombMaze[MAZE_W][MAZE_H];
extern Explosion *explList;
...
def_score_extra (BMPlayer *ps)
{
  ...
  if ( specialExtraFunc ==SpecialExtraSlow){
    ...
    else{ if(!ps->revextra_flags)
      bonus_extra = -BONUS_SIMPLE / 2;
    else bonus_extra = 0; /* Si je suis déjà lent, reprendre une tortue ne change rien */
    }
  }
  ...
  /* La seringue */
  if ( specialExtraFunc == SpecialExtraJunkie &&
      ps->junkie == 0){
...
score_explose (int x, int y, int portee_bombe)
{
  ...
  for (dir = 1; dir < NB_DIR; dir++)
  {
    ...
    else explose -= (score_bonus (x + i * dx, y + i * dy) / 4);
  }
  explose+=75;
  return explose;
}
...
teste_case (coord *c, int dir_init, BMPlayer *ps)
{
  ...
  { 
    ...
    if (!laby_visit[c->x + dx][c->y + dy] &&
	!CheckMaze (c->x + dx, c->y + dy)
	)
    {
      if(!ps->kick){
...
calc_laby_perf()
{
  ...
  for (temps = 0; temps < LONG_FUSE; temps++) { /* On deplace case par case */
	...
	if (p->count == (fuse_times[curBombTime]*3/5)) {
	  ...
	  for (j=-1; j <=1; j++) {
	    if ((p->x+j < MAZE_W) && (p->x+j > -1) && !CheckMazeSolid(p->x+j,p->y)) {
	    ...
	    if ((p->y+j < MAZE_H) && (p->y+j > -1) && !CheckMazeSolid(p->x,p->y + j)) {
	      /* On trouve un emplacement libre ... */
	      for (k = 0; k < 100; k++)
		if (bombes_tmp[k].range == -1)
		  break;
...
init_laby_perf ()
{
  ...
  for (p = explList, i = 1; p != NULL; p = p->next, i++) 
    bombes_tmp[i] = *p;
...
dirige_vers_bonus (int x, int y, int dir)
{
  ...
  conversion_direction (dir, &dx, &dy);
  if((CheckBonuses2 (x + dx, y + dy)) && (specialExtraFunc ==SpecialExtraPoison))
    { //fprintf(stderr," field  poisoned\n");
      return -1;// POISON!!! 
    }
  else{
...
choix_direction (int *choix_dir, int *max_perf)
{
  ...
  *max_perf = 0;
  for (dir = 0; dir < NB_DIR; dir++)
  {
    ...
    if(dir==old_dir && dir==0) perf_max[dir]-=100;
  ...
  for (dir1 = 0; dir1 < NB_DIR; dir1++)
  {
    if(perf_max[dir]==perf_max[dir1] && dir!=dir1){
      if(dir<dir1){
	if(dir==old_dir)perf_max[dir]+=75;
	else perf_max[dir]++;
...
test_biscotte (BMPlayer *ps)
{
  ...
  for (bombe = explList; bombe != NULL; bombe = bombe->next) 
...
actions (BMPlayer *ps)
{
  ...
  if (souhaite_poser_bombe (ps))
  {
	...
	conversion_direction (choix_dir, &dx, &dy);
      	if(!CheckBomb(x-olddx,y-olddy))i=0;
...
/* Fouf lenteur ++ */ /* fflush(stderr); */
  ...
  if (ps->illness == IllReverse)
    inverse = XBTrue;
  else inverse = XBFalse;

  /* Si je me trouve entre deux cases */
/* Fouf lenteur -- */ /*  if ((ps->x % BLOCK_WIDTH) != 0 || (ps->y % BLOCK_HEIGHT) != 0) */
/* Fouf lenteur ++ */  if (((ps->x % BLOCK_WIDTH) != 0 || (ps->y % BLOCK_HEIGHT) != 0) || ((ps->d_ist == GoStop) && (temps_jeu % 3)))
  {
    switch (ps->d_ist)
    {
    case GoUp:
