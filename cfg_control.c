 ...
 * 
 * $Id: cfg_control.c,v 1.8 2004/08/05 19:51:42 iskywalker Exp $
 *
 * Program XBLAST 
...
static const XBEventCode keyEventType[NUM_KEYB_CONTROLS] = {
...
{
  switch (type) {
  case XBE_KEYB_1: return atomRightKeyboard;
  case XBE_KEYB_2: return atomLeftKeyboard;
...
/* Skywalker */
      ...
      num += PutEntry (keyTable + num, type, XBGK_CHAT_SEND,        section, atomKeyChatSend); 
      num += PutEntry (keyTable + num, type, XBGK_CHAT_START,        section, atomKeyChatStart); 
      num += PutEntry (keyTable + num, type, XBGK_CHAT_CANCEL,        section, atomKeyChatCancel); 
      num += PutEntry (keyTable + num, type, XBGK_CHAT_CHANGE_RECEIVER,        section, atomKeyChatChangeReceiver); 
    /* */
    }
  }
  /* game control keys */
